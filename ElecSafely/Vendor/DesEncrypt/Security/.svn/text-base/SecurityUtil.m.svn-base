...
#define Iv          @"1a2b3c4d5e6f7g8h" //偏移量,可自行修改
#define KEY         @"1a2b3c4d5e6f7g8h" //key，可自行修改

@implementation SecurityUtil

#pragma mark - demo start 
- (void)example {
    //加密
    NSString *encryptDate=[SecurityUtil encryptAESData:@"testmessage"];
...
+ (NSString*)encryptAESData:(NSString*)string key:(NSString *)key
{
    ...
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    //使用密码对nsdata进行加密
    NSData *encryptedData = [data AES128EncryptWithKey:key gIv:key];
    //返回进行base64进行转码的加密字符串
    return [self encodeBase64Data:encryptedData];
}

#pragma mark - AES解密 带自定义的Key
//将带密码的data转成string
+ (NSString*)decryptAESData:(NSString *)string key:(NSString *)key
{
    ...
    NSData *decodeBase64Data=[GTMBase64 decodeString:string];
    //使用密码对data进行解密
    NSData *decryData = [decodeBase64Data AES128DecryptWithKey:key gIv:key];
    //将解了密码的nsdata转化为nsstring
    NSString *str = [[NSString alloc] initWithData:decryData encoding:NSUTF8StringEncoding];
    return str;
}
